...
No policy option exists (corrective-spec-001 CS-2): the schema and the Git
root are resolved by the validator, never by the caller. Exit 0 holds, 1
rejected, 2 usage (including a closed record without ``--diff``), 3 could not run.
"""
...
from devforgeai.checkpoint.validate import CouldNotRun, RangeRequired, validate_plan
...
def _parser() -> argparse.ArgumentParser:
    ...
    validate.add_argument("--diff", default=None,
                          help="Git range <base>..<head>; mandatory once any record is closed; "
                               "head must be HEAD and base a proper ancestor")
