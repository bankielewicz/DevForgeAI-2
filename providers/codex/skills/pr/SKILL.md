...
Read `references/capability.md` and `references/workflow.md`. Accept exactly
...
`devforgeai status` and dispatch the custom agent named by the status block.
Pass the verbatim status block and no diff content. The hook-owned SubagentStop
path ingests the receipt and advances the run.
