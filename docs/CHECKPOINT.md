...
## Check-in 18 — 2026-09-04, D17 proven live (run 5, build 4, both runtime probes pass); plan re-freeze unblocked

**Live run 5** (`~/Projects/dfai-proof`, branch `run5`, base `6bb06b8`, session f639832e, 11:29–11:45Z, about 16 minutes): sequencer from `10151a2` (PR 14, D17), settings fragment from PR 15, build-4 skill (spec at `10151a2`, evals 7/7, 4/4, 5/5, `#7-procedure` `sha256:055e8d25…b6b4`) and its compiled agents, worktree mode. Five phases passed, two findings persisted (4,008 and 9,107 bytes) and listed in the handoff, the first handoff printed verbatim, promotion fast-forwarded `run5` to the review checkpoint, and the three tests pass in the canonical tree. 120 raw hook events (69 PreToolUse, 5 SubagentStart, 31 SubagentStop of which 26 identity-less helper events, all ignored), no hook fault.

**Probe 1, read-only git as `git -C <candidate.root>`: PASS.** Fourteen `git -C <root> status|diff|log` calls across all five workers (red 2, green 2, refactor 1, smoke 1, review 8) were admitted by the dispatcher and by the provider allowlist; no permission refusal appears in any transcript. The run's only two dispatcher denials were `find`, which is outside the read-only set, both recovered by the worker. Not exercised live: a `-C` naming the canonical checkout or a mutating subcommand, because no worker attempted one; the thirteen conformance rows remain the evidence for the hostile cases.

**Probe 2, sequencer-rendered `## Oracle`: PASS.** Every phase report carries the section rendered from `<phase>-result.json#oracle`: red `EXPECTED_TEST_FAILURE` with the three criteria `failed`; green and refactor `PASS` with the three `passed`, refactor adding `lint: PASS`; smoke and review `report_only`, "no key brokered at this phase", carrying forward `test PASS`. No `## Oracle problems` heading anywhere. Both judges cited `result.json#oracle` (smoke once, review six times) rather than the worker's note.

**Consequence**: D17 is PROVEN on the Claude side for both changed runtime surfaces, positive paths live and hostile paths by conformance. Every gate the owner set before the plan re-freeze has passed: a build at 7/7, 4/4, 5/5 from the merged spec, a live run with the generated skill, and the D17 probes. **Task-list item 2 is unblocked**: Codex executes the mechanical re-freeze of the gap-closure plan against `564ce89` (the PR 15 merge), the human remains decision authority, and this session reviews the re-freeze independently.

**On record, not blocking**: identity-less helper subagent events continue at the same rate (26 in this run); the worker-side `devforgeai run test` label `TEST_FAILURE` versus the oracle's `EXPECTED_TEST_FAILURE` stays a known draft behaviour; the gate ran `--lenient` over placeholder hashes, as every fixture run does.

## Check-in 17 — 2026-09-04, D17 pre-freeze stabilization (IMPLEMENTED; provider probes open)

**Owner's rulings after check-in 16**: all four D17 candidates fixed in one bounded PR as the final pre-freeze wave; the Oracle section rendered from sequencer-owned results, never copied from the worker note; positive and hostile `git -C` cases including the wrong root and mutating subcommands; after merge, regenerate the hash-bound skill once and run the existing evals plus focused live probes for the two changed runtime surfaces; further observations block the re-freeze only on correctness, isolation, provenance, security or an existing normative promise; Codex executes the mechanical re-freeze against the D17 merge commit, the human stays decision authority, and this session reviews it independently. Item 2 was held until D17 lands, because an earlier re-freeze would have been stale at once.

**Made** (branch `worktree-d17-stabilization`, base `8a1ce98`, register D17): the dispatcher admits read-only git as `git -C <candidate.root> <subcommand>` (once, first, exact path) and denies every other `-C` form and every mutating subcommand under it, with thirteen new conformance rows; the sequencer records every key it brokers at ingest into `<phase>-result.json#oracle` and renders the report's `## Oracle` from that record alone, with a backstop proving a worker's "99 passed" note never reaches the section; spec 001 section 10 carries `[--fix]`, section 9's judge sentence names the read-only set and the `-C` form, section 7e and 6 updated; doc 09 check 7, doc 10's report-rendering sentence and `templates/agent-md.md` state the same. Hashes recomputed for the specs that cite the template.

**Verified locally**: conformance 240/240 (147 dispatcher, 35 grammar, 58 backstops); `demo_sequencer.sh` green for Python and Node in copy and worktree modes; hookd 23/23; `verify.py` V1, V2, V3, V4, V8, V9 ok on 18 specs; `git diff --check` clean.

**Addendum, 2026-09-04, after the PR 14 merge (`10151a2`)**: build 4 of the dev skill from the merged spec passed: no SPEC GAPS, section 14 clean, evals 7/7 (936 s), 4/4, 5/5; `#3-description` `sha256:0dc4136b…cebb` unchanged and `#7-procedure` moved to `sha256:055e8d25…b6b4`, both recomputed here against `10151a2` and equal. Gate (a) below is therefore passed. The grader saw a `green_dev` call to `git -C <root> status --short` refused by Claude Code's own permission layer before any hook ran (headless: no prompt is possible): the settings fragment allowed `git status *` and friends but nothing beginning `git -C`. Fixed in the fragment: `Bash(git -C *)` and `Bash(git rev-parse *)` are allowed, the hook (which runs first) remaining the bound. The other grader observations (first handoff printed without its `evidence:` lines; worker-side TEST_FAILURE versus the oracle's EXPECTED_TEST_FAILURE; one call-time COLLECTION_ERROR label; both red attempts inside one subagent) are on record and none violates correctness, isolation, provenance, security or a normative promise. The `model: inherit` key written into the package's source agent files is a permitted deviation (7g places it in the compiled file; the compiler copies it).

**Open, provider-facing, after merge**: (a) passed, see the addendum; (b) live probe 1: a judge runs `git -C <candidate.root> status` and `log` and is admitted, and the same command naming the canonical checkout is denied; (c) live probe 2: a judge cites `<phase>-result.json#oracle` or the report's `## Oracle` section instead of the worker note; (d) then Codex re-freezes the gap-closure plan against the D17 merge commit and this session reviews it. Not pinned by a conformance row: the `## Oracle` section's report-only branch (smoke and review, "no key brokered", `carried_forward`); the demo renders it and live probe 2 reads it.

## Check-in 16 — 2026-09-04 (UTC), generated dev skill proven live: build 7/7, 4/4, 5/5 and live run 4 promoted

**Build 3 (owner-run, fresh session, spec at `a94af38` after PR 12)**: no SPEC GAPS; section 14 checks clean; `SKILL.md` 105 lines; both judges without a write tool; quick evals 7/7 (771 s), 4/4 (12 s), 5/5 (207 s). Provenance `#3-description` `sha256:0dc4136b…cebb` and `#7-procedure` `sha256:406f9c8b…abfc` were recomputed here against the merged spec and are equal. The previous 6/7 package is kept at `out-2395f3a/`. This closes the acceptance the upper check-in 14 entry and check-in 15 left open: the package is admitted as current acceptance evidence.

**Live run 4** (`~/Projects/dfai-proof`, branch `run4`, base `aca9a14`, session 71608061, 02:18–02:39Z, about 21 minutes): the first run of the generated skill and its compiled agents, in worktree mode (the evals ran in copy mode), on the D16 sequencer. The gate opened under `--lenient` with five placeholder-hash warnings; five workers ran in order and each bound its lease at SubagentStart; red wrote the three planned tests (TEST_FAILURE from the worker's own `devforgeai run test`, assertion failures at ingest), green modified `tinyapp/text.py` (PASS, test-file hash equal to red's), refactor changed nothing (test and lint PASS), smoke and review returned `findings` (5,492 and 9,237 bytes) that the sequencer persisted and listed in `findings_paths`; `granted_keys` read `[]` at review as the D16 intersection rule requires; the first handoff block was printed verbatim this time; the owner confirmed, promotion fast-forwarded `run4` to the review checkpoint and removed the root, and the three tests pass in the canonical tree. 135 raw hook events (101 PreToolUse, 5 SubagentStart, 23 SubagentStop of which 18 identity-less helper events, all ignored), no hook fault. 18 dispatcher denials, every one recovered by the worker within the grammar: 14 refusals of `git -C <root> status|log|diff` and 4 of shell operators.

**Consequence**: the Claude side of the dev loop, one Python story in worktree mode, is PROVEN with a skill-creator-generated skill rather than a hand-written stand-in. Both provider-facing gates the owner set for item 1 have passed; item 2 (rebase, re-freeze and accept the gap-closure plan against `a94af38`) may start.

**D17 candidates, for the owner's ruling (nothing changed here)**:

1. Dispatcher: `check_git` takes `argv[1]` as the subcommand, so `git -C <candidate.root> status` is refused as "mutates history" although doc 10 and spec 7e promise read-only git inside the root. A worker's cwd is the canonical checkout (a subagent inherits it), so `-C <root>` is the only way to ask. Proposed: parse `-C <path>`, require the path to equal `candidate.root`, then apply `GIT_READ_ONLY`; conformance rows for the positive case and for `-C` pointing elsewhere.
2. Spec 001 section 10's success-criteria grammar omits `--fix` while sections 7a, 11 and 13 include it.
3. Spec 001 section 9 says a judge's Bash is `devforgeai status` alone while 7e and 7g grant the dispatcher's read-only set too; the generator followed 7e/7g.
4. Rendered phase reports carry the oracle classification only inside the worker's note; both judges asked for a sequencer-rendered oracle section (`dev-notes` template and renderer).

Observations, on record only: eval 2's primary appended a sentence after the sequencer's `Next:` line; eval 3 ran both red attempts inside one subagent (the exit-2 SubagentStop keeps the worker going, already known) and one worker-side `devforgeai run test` was classified COLLECTION_ERROR while junit showed failures only, resolved on rerun (sequencer draft; to reproduce).

## Check-in 15 — 2026-09-03 night, second cold-session build 6/7 (historical, not admitted); D16 corrections

**Build (owner-run, fresh session)**: skill-creator quick mode from SKILL-SPEC-001 at merge commit `2395f3a` (PR 11), `claude -p` evals in fixture copies. No SPEC GAPS; `quick_validate` valid; `SKILL.md` 106 lines; both judges `writes: none` with no write tool; evals 6/7, 4/4, 5/5. The single failure was spec-internal: eval 1 expectation 3 forbade the rendered report views section 6 declares, so it failed on every correct run. The build also exposed a sequencer defect: the `refactor` status block advertised `granted_keys [build, lint, test]` while `devforgeai run build` was refused because the story's `commands.use` is `[test, lint]`. The `out/` package is historical evidence bound to `2395f3a`; it is not admitted as acceptance and will not be regraded or overwritten. It supersedes the "predates D13, 2/7" line in the upper check-in 14 entry, which stays as written.

**D16 (branch `worktree-d16-spec001-round2`, base `2395f3a`)**: `granted_keys` is computed once as the phase's registry run keys ∩ the story's `commands.use` (`policy.granted_keys`) at every phase entry; state, the status block, the dispatcher's `PreToolUse` check and `devforgeai run` read that one stored value, and a run file without the key admits nothing. Spec 001: expectation 3 now permits the write fence, sequencer-owned `.devforgeai/**` and the declared `docs/reports/**` views; `input_checkpoint` enum closed as `base | red | green | refactor | smoke`; character count 848; duplicated `Bash` removed; the `>` after `description:` stated as YAML syntax; the section 14 compound scan limited to fenced blocks and code spans; `# dev` and `# envelope` headings stated; section 7g quotes the template's judge sentence in full; section 7e states that the status block's key set, not the contract's, is the run's. Handoff paraphrase, judge reads outside its inputs list, and the missing `python`/`skills-ref` are recorded as observations. Register: `WRITE-MODEL-REVISION.md` D16.

**Verified locally**: conformance 226/226 (134 dispatcher, 35 grammar, 57 backstops; four new rows and one new backstop); `demo_sequencer.sh` green for Python and Node in copy and worktree modes; hookd 23/23; `verify.py` V1, V2, V3, V4, V8, V9 ok on 18 specs, hashes recomputed (doc 09's count line is outside every cited section); `git diff --check` clean. Status: IMPLEMENTED, not PROVEN.

**Next, in the owner's order**: merge the D16 PR; move the existing `out/` aside first (for example to `out-2395f3a/`), because the cold-session prompt writes to `./out`; generate a completely fresh package there from the merged spec (never regrade or overwrite the previous one); require 7/7, 4/4, 5/5; reset `~/Projects/dfai-proof` to the fixture stub and run the second live proof with the generated skill; only after both provider-facing gates pass, rebase and re-freeze the gap-closure plan (task list item 2).
...
**D13/D14/D15 landed (2026-09-03 evening)**: judges `writes: none` with no write tool; receipt `findings` (required on a judge's pass/fail, optional on needs_user/could_not_run, forbidden on producers, 16384-byte bound, never truncated) persisted by the sequencer to the run's evidence directory and listed in the handoff's `findings_paths[]`; reason codes `provider_tool_refused` (INFRA_FAILURE), `prerequisite_missing` (COULD_NOT_RUN), `checkpoint_fault` (INFRA_FAILURE); per-skill handoff rows so red's "already passes" routes to `/clarify`; `phase start` accepts `--fix` and `--lenient` with per-skill fix-report sources (`NO_FIX_SOURCE`, `NO_FIX_REPORT`); `clarify` exempt from STORY_IN_FLIGHT; resume re-gates and re-slices when the story hash changed; the seven spec-001 defects and two SKILL.md defects fixed; the 16 KiB bound recorded as a real limit for tree-enumerating judges (evidence broker PM-14). **Battery**: verify V1–V4, V8, V9 ok with hashes recomputed; conformance 222/222 (131 dispatcher, 35 grammar, 56 backstops); demo green in all four runs; hookd 23/23; taxonomy valid; contracts manifest 7/7. Proof project reinstalled and reset on branch `run3` for the fresh-session judge persistence test (D13 item 8).
