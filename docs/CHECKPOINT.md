...
**Open, provider-facing, after merge**: (a) move `out/` aside and regenerate the dev skill from the merged spec (sections 6, 7e, 9 and 10 changed, so the `#7-procedure` provenance hash moves), evals must stay 7/7, 4/4, 5/5; (b) live probe 1: a judge runs `git -C <candidate.root> status` and `log` and is admitted, and the same command naming the canonical checkout is denied; (c) live probe 2: a judge cites `<phase>-result.json#oracle` or the report's `## Oracle` section instead of the worker note; (d) then Codex re-freezes the gap-closure plan against the D17 merge commit and this session reviews it. Not pinned by a conformance row: the `## Oracle` section's report-only branch (smoke and review, "no key brokered", `carried_forward`); the demo renders it and live probe 2 reads it.
