...
2. `components/research-core/src/devforgeai/checkpoint/`: the semantic validator, invoked as
   `PYTHONPATH=components/research-core/src python3 -m devforgeai.checkpoint validate --plan docs/research/spec-driven-development-gap-closure`. Rules S01–S13 are listed in its module docstring. Exit 0 holds, 1 rejected, 2 usage, 3 could not run.
...
5. `framework/contracts/PROMOTION-CANDIDATE.md` slice 2 and `framework/contracts/MANIFEST.sha256`: the promotion candidate the CP-00 record pins (amendment SDD-GAP-AMD-001).
6. This dossier and its manifest.
