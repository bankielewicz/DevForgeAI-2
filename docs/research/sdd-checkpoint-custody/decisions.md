...
## D-CP00-13 — Halt at `257ba7d`: candidate `89be834` is not promotion-eligible; corrective 002 ordered

The Codex exact-head review of PR 22 (`reviews/codex-security-2026-09-04-pr22/`, snapshot `sha256:fa2e218fa6da5498e0eea5cc899a20bb1cdfca48fe70e14745ccaec64018a56b`) returned **FAIL / PROMOTION_HALT** with four high findings, each with a recorded hostile reproduction:

1. `_git` inherits the caller's environment and resolves `git` through `PATH`; `GIT_DIR`/`GIT_WORK_TREE` selected another repository's `HEAD` while `--show-toplevel` still reported the legitimate work tree.
2. CS-1.8 confirmed: the executing release verifies itself and the record's release verifies separately; nothing requires the same root and executable, and version, source commit and promotion evidence are not bound to the executing release. A two-root fixture returned PASS.
3. The shell wrapper cannot scrub its pre-start environment: an `LD_PRELOAD` constructor ran before `bin/devforge`'s first line, without changing protected bytes.
4. CS-3.3 exploited: implementation work committed after a legal closure, the closed record re-pinned to it, and a caller-selected base after the implementation commit returned PASS with no S10 finding.

Dispositions: V-03 FAIL (C-01 fails: condition 4 is only partly decidable per C-04, condition 9 is not decided; C-02 to C-07 pass) so `researched` stays `NOT_RUN`; layout contract v1 not confirmed (launcher, exact modes, release identity and source commit not equivalently enforced); the plan README's CP-00 required outputs are to be updated; D-CP00-11 stays in force.

Decision authority rulings (2026-09-04), verbatim where they bind:

- Corrective 002 is appended to PR 22's branch as new commits; `686fb9a`, `89be834` and `257ba7d` are never rewritten, so the reviewed head stays in history. PR 22 merges only after a PASS at the new head, with a merge commit.
- Finding 3: "Architect devforge as the permanent Rust trust-boundary binary, not a temporary C launcher. Initially, devforge checkpoint validate may delegate to the Python validator; later the validator moves behind the same Rust CLI without changing hooks or invocation." Requirements: a genuinely static Linux binary for `x86_64-unknown-linux-musl`, verified to have no ELF interpreter or dynamic dependencies; no shell and preferably no third-party crates; `rust-toolchain.toml`, `Cargo.lock`, source, build script, compiler identity and resulting binary digest pinned; the release root determined from the running executable and the record's executable required to equal that exact `<root>/bin/devforge`; `Command::env_clear()` with an explicit minimal environment, executing absolute `/usr/bin/python3` for now; `_git` independently sanitized and invoking `/usr/bin/git` ("the launcher alone should not be the only protection"); hostile tests for `LD_PRELOAD`, `LD_AUDIT`, Git variables, wrong release root and dynamic linkage; "The Rust host must eventually supply the trusted review range; a caller-provided --diff remains insufficient."
- Finding 4: identity binding **and** a root-owned range attestation. "The protected Rust devforge binary—not the agent—mints the attestation after the closure PR head is frozen. It records repository identity, plan/checkpoint ID, actual PR base and head, record digest, candidate/release identity digest, authority, and timestamp. Store it in a fixed root-owned directory; do not accept a caller-selected attestation path. The validator obtains the range from it. If --diff remains, it must exactly match the attested range. Any head, record, candidate, or release change invalidates the attestation and requires a new independent review. Record its digest in the PR review/check rather than modifying the attested commit and creating a circular hash. Missing, stale, mismatched, or replayed attestations fail closed." Identity binding "is necessary, just not sufficient."
- C-01 is revised (claims.jsonl revision 2) and stays `NOT_ACCEPTED`.

Effect on the record: `implemented: NOT_RUN`, `trust_stage: UNBOUND` with a null pin (as after D-CP00-11); the limitation names `89be834` so the reviewed defective bytes stay identified; the candidate is recorded as not promotion-eligible. The corrective specification is `corrective-spec-002.md`; its hostile tests are written first and recorded failing on this candidate (`probes/red-corrective-002.txt`).
