...
Every anatomy-governed non-Research skill owns a dedicated set of workers. Research is the explicit exception governed by `capabilities/research/`; it defines contracts for four bounded worker roles that write nothing and deterministic Research Core as sole canonical writer. Current provider templates do not execute those workers. This document lists the sets and gives `dev-tdd` as the worked example.
