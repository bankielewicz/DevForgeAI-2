...
`pr_drafter` is a producer with `writes: candidate`; its entire write set is
`pr-artifacts/title.txt` and `pr-artifacts/body.md` inside the run's candidate
root. `pr_critic` is a judge with `writes: none`; it checks only the pinned
committed range and those two artifacts, returns findings in the receipt, and
never edits the draft. A failed critique rewinds to `pr_drafter`. Neither
worker may push, call a GitHub API, merge, release, install, read secrets, or
