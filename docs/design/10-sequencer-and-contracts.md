...
2. **No worker writes a canonical byte.** A producer writes real files with Edit and Write, inside the candidate root and inside the fence, under a lease bound to its own start event. Those bytes reach the canonical tree only through `devforgeai promote <run>`, which refuses on a moved base. A judge writes nowhere at all: it declares `writes: none`, carries no `Write`, `Edit` or `apply_patch` tool, and returns its evidence as the receipt's `findings`, which the sequencer persists to `.devforgeai/work/<run>/evidence/<agent>/findings.md` on the judge's behalf. Neither role can touch a tracked file that a promotion has not moved.
...
| `NO_FIX_REPORT` | `phase start` | `--fix` was given and neither `docs/reports/qa-<story>.md` nor `docs/reports/review-<story>.md` exists, so there is no report for the run's workers to read and nothing to record as `run.yaml#fix_report`. The refusal names both paths it looked for |
...
| Reason code | `runner_missing`, `timeout`, `network`, `hook_fault`, `provider_tool_refused`, `prerequisite_missing`, `checkpoint_fault` | the worker, or the sequencer when it synthesises a result | required with `could_not_run`, rejected with every other status |
...
`needs_user` never retries. It is recorded, written into a `REQUIRE_HUMAN` handoff, and the run is blocked at that phase: status stays `active`, the lease is released, the candidate root survives, and `run.yaml#blocked_at` names the phase; the attempt counter is not consulted. A worker that wants a human decision gets one on the first ask. The run resumes when the user has acted: `devforgeai phase start <skill> <arg>` with the same skill and argument on a blocked run **resumes** it at `blocked_at` with `attempts` reset to zero instead of refusing `STORY_IN_FLIGHT` (the same rule resumes an attempt-limit block). `clarify` is the one other skill that may open against that story while the run stays blocked, because its fence is the story document under `docs/plan/` and not the code the blocked run holds (section 12.2); `review`, `qa` and every other skill still need `devforgeai phase fail --reason <text>` first, which abandons the root.

Resuming re-gates. `/clarify` is the reason the story a blocked run holds can change under it, so `devforgeai phase start dev <story>` — and the same call for any other blocked story or story-anchored run — on a blocked run whose story file no longer hashes to `run.yaml#story_sha256` — the digest of the story file as it stood at `phase.start` — re-runs the whole story gate of section 3.2 and re-slices `work/<run>/context.json` from the current story before it re-enters `blocked_at` and resets `attempts`. The candidate root, its checkpoints and its `base_ref` are untouched by that: only the gate verdict, `run.yaml` and the context slice are recomputed. If the re-run gate refuses, nothing is reset and the run stays blocked at the same phase with the gate's reasons printed, so a `/clarify` that made the story worse cannot silently restart the phase. A story whose hash is unchanged skips the re-gate and resumes as before.
...
| `devforgeai ingest-result` | not valid JSON, no receipt declaring the schema, more than one such object, unknown or missing keys, oversize (over 64 KiB total, over 64 `claimed_paths`, over 16 `evidence_refs`, note over 16 KiB, `findings` over 16384 UTF-8 bytes or missing from a judge or present from a producer, over 10 issues): refuse, exit 1, dispatcher exit 2, the same worker continues. The attempt counter is **not** incremented: a malformed receipt is a protocol error, not a phase attempt. A change set that exceeds no cap but is not a subset of `claimed_paths` is `UNCLAIMED_CHANGE`, which **is** a phase attempt: the bytes are real and the phase must redo them | inherited from the dispatcher's 660 s bound |
| Transition oracle | JUnit XML that does not parse: `COLLECTION_ERROR`; `junit_path` present but no test cases: `NO_TESTS`; a test case the section's `junit_dialect` reads as a throw rather than a failed assertion: `COLLECTION_ERROR` | brokered command over `timeout_s` (default 600): `TIMEOUT`, mapped to `could_not_run` / `timeout` and routed by `gate_policy.test_runner_missing` |
...
The oracle reads JUnit in one vocabulary — `<failure>` is a test that ran and failed an assertion, `<error>` is a test that never ran — and the red gate is built on it: `check_red` requires every `test_plan` name to be present and failed, because a phase that owes a failing test has not delivered one if the test merely threw. Real runners do not all write that vocabulary, and a JUnit file does not say which runner wrote it, so the section's `junit_dialect` (section 7) names the runner and the oracle normalises the file before it classifies. Three shapes were measured against live reporters and each one, read literally, admits something the gate exists to refuse: node's built-in runner reports an assertion failure, a `ReferenceError` thrown in a test body, and a module that would not parse all as `<failure type="testCodeFailure">`, and only the assertion spells out `cause: AssertionError [ERR_ASSERTION]` in the element body; node reports an empty test file as one *passing* test case named after the file, so an empty red suite reads as a green one; and pytest reports a `NameError` raised in a test body as a `<failure>` distinguishable from a real assertion only by its `message`. Under `node`, a failure carrying no assertion marker is read as `error`, a case named after a test file is a whole-run `COLLECTION_ERROR` when it failed and `NO_TESTS` when nothing ran under it; under `pytest`, a failure whose message does not begin with `AssertionError`, `assert ` or `Failed:` is read as `error`, and a message-less failure is read as `error` too. Normalisation is monotone: it moves an outcome towards `error` or drops a case that never ran, never the reverse, so a dialect can refuse a transition the literal reading would have admitted and can never admit one the literal reading refused. `generic` is the default and reads the file exactly as it has always been read.
...
The registry in `policy.py` is the single source of truth for which phases a skill has, in what order, which worker each dispatches, what it may write, how many attempts it gets, which stack keys it grants, which oracle runs at its transition, which earlier phase it may rewind to, and which handoff row each problem kind selects. The table below is that registry; the `handoff` rows are per skill and are described in section 6.

Column meanings:

- **writes** — what the phase's worker may change inside the candidate root: `docs` (the run's fence, document mode), `tests` (only paths in `test_paths`), `code` (fence paths that are not in `test_paths`), `fields` (a narrower path set inside the run fence, and only the frontmatter keys the phase declares; see below), `none` (a judge: nothing in the root, and any derived change is a refusal). The column is also the role split: every mode but `none` is a producer, which holds the run's write lease while it is dispatched; `none` is a judge, which holds no lease and may run against a checkpoint while another judge does. `none` is literal: a judge carries no `Write`, `Edit` or `apply_patch` tool and creates no file anywhere. It returns its evidence as the receipt's `findings`, and the sequencer writes that string to `.devforgeai/work/<run>/evidence/<agent>/findings.md` at ingest (section 5). The directory is outside every candidate root, so nothing a judge produces enters a diff, a checkpoint or a promotion. A worker header declares the same split as a closed enum, `writes: candidate | none`: `candidate` for every producer mode in this column, `none` for a judge. A `docs` phase marked **conditional** owes its document only under a condition the run may not meet, so the document oracle accepts an empty change set from it when the receipt's `note` says why none was owed; every other `docs` phase must change a file.
...
One schema, both providers, every skill. The worker's final message is exactly this object, with no Markdown fence and no surrounding prose. It is a **receipt**: it says what the worker did, not what it wants done. A producer has already written its files inside the candidate root; a judge has written nothing anywhere and carries its evidence in `findings`.

What crosses this boundary is exact. No file body, no diff and no hash does: a producer never returns the bytes it wrote, so `plan`'s `stories` phase can write a slice of any size and a `green` phase can touch a binary fixture. A judge's `findings` string does, because a subagent returns its result to its parent — a hook can validate that final message but cannot suppress it — so the bounded `findings` body necessarily enters the primary context as part of the subagent's result. That is why it is capped at 16384 UTF-8 bytes. What stays isolated is the worker's transcript, its file reads, its tool traffic and its intermediate reasoning; none of that reaches the primary window on either provider.
...
| `evidence_refs` | array | no | at most 16 paths to evidence the sequencer should read: a report inside the root, or an oracle output the sequencer itself wrote under `.devforgeai/work/<run>/`. A report-producing phase names its report here, and that report's frontmatter `verdict` selects the handoff row (section 6). A judge names no findings path here; it has none to name, because the sequencer chooses the path. Never a body, never a hash |
| `note` | string | no | at most 16384 bytes; three lines is the intended length |
| `findings` | string | conditional | the judge's detailed evidence, as prose. **Required** when the active phase's registry `writes` mode is `none` — that is the deterministic test for "the worker is a judge" — and the status is `pass` or `fail`; optional on a judge's `needs_user` or `could_not_run`; **forbidden** on a producer receipt, on every status. At most 16384 UTF-8 bytes. The sequencer never truncates it: an oversize `findings` refuses the receipt like any other defect. At ingest the sequencer writes the decoded string verbatim to `.devforgeai/work/<run>/evidence/<agent>/findings.md`, records that path as `findings_path` in `<phase>-result.json`, and lists it in the handoff's `findings_paths[]`; the next phase's worker reads it by that path rather than being handed its text. `issues[]` stays the bounded routing summary |
...
Steps 1 to 8a read the receipt alone. Step 8b is the one ingest step that writes on any status, and it writes only outside every candidate root: it persists a judge's `findings`. Step 9 is where the sequencer stops believing the worker and looks at the tree: it diffs the candidate root against the receipt's `input_checkpoint` and derives
...
| 8a | `findings` rules, decided by the active phase's registry `writes` mode alone: when the mode is `none` and the status is `pass` or `fail` the key is present, is a non-empty string, and is at most 16384 UTF-8 bytes measured after decoding; when the mode is `none` and the status is `needs_user` or `could_not_run` the key is optional under the same bound (a judge stopped by the provider may have nothing to report); when the mode is anything else the key is absent. Nothing here is truncated, defaulted or repaired — an over-long, empty or misplaced `findings` is a receipt defect | refuse |
| 8b | persist `findings`, on a judge phase only and on every status. The sequencer creates `.devforgeai/work/<run>/evidence/<agent>/` if it does not exist and writes the decoded string verbatim to `findings.md` inside it, `<run>` being the active run and `<agent>` the canonical name of the phase's worker. The worker chooses neither the directory nor the file name and never held a write tool; this is the sequencer persisting a returned result, not a merge of a worker's snippet into a tree. The path is recorded as `findings_path` in `<phase>-result.json` (5.3) and listed in the handoff's `findings_paths[]` (section 6), and that is how the next phase's worker is given it | the sequencer's own fault, not the worker's: when the directory cannot be created or the file cannot be written, exit 3 and record the phase as `could_not_run` with no worker reason code |
| 9 | derive `changed[]` from the checkpoint diff, as above | refuse when the checkpoint is missing or the diff cannot be taken (`could_not_run`, `checkpoint_fault`) |
...
| 11 | per changed path: canonicalised, inside the candidate root, not sequencer-owned, inside `write_fence`, allowed by the phase's `writes` mode — for `red` that means inside `test_paths`, for `green` and `refactor` inside the fence and outside `test_paths`, and for a judge (`writes: none`) that `changed[]` is empty, because a judge holds no write tool and creates no file anywhere | refuse |
...
| Result | `findings_paths[]` | no | one path per judge phase of this run whose receipt carried `findings`, from each `<phase>-result.json#findings_path`. The envelope carries the path, never the text; a later phase's worker and a human reader open `.devforgeai/work/<run>/evidence/<agent>/findings.md` there |
...
| 9 | A row whose `next` names another skill's command is a handoff, never a call. No skill invokes another skill's run: `phase start` refuses while a run is active, and refuses a story that another unpromoted run already holds, and the promotion that clears the run has not happened by the time the block is printed, so the named command is run afterwards by a human or a fresh session. The `/clarify <story>` row is the one case where the earlier run is still open when the named command runs, and it is still not a call: the run is blocked, holds no lease, and `clarify` opens a run of its own against the story document (section 12.2). A specification's procedure may not describe a nested run, and the sequencer never starts one. |
...
Handoff-row selection is per skill, not a framework default. The registry (section 4) carries a `handoff` row set in `policy.py` for each skill, keyed by problem kind — the refusal token, the worker status and reason code, the oracle classification, or the promoted-and-clean case. Every refusal therefore names its own row, and the row the sequencer selects is the one the skill's specification section 7f names for that problem kind. The sequencer does not fall back to a generic table when a skill declares a row, and a skill that declares none for a kind is a specification defect, not a licence to improvise.

The worked example is red's "a planned test already passes". `dev`'s section 7f gives that problem kind the row whose `next` is `/clarify <story>`, because the story's test plan is what is wrong; the sequencer selects that row and prints that command. The earlier behaviour — routing it to the generic `BLOCK` row and printing `/dev <story> --fix` — was the sequencer ignoring the skill's table, and it was wrong.

The rows below are the framework-level situations, and they are what a skill's `handoff` set is layered over. The three verdict rows are themselves per-skill: they are the rows `review`, `qa` and `skill-validator` name in their own section 7f tables, printed here because those three skills are the only report-producing ones.
...
| a problem row beginning `COULD_NOT_RUN: hook_fault` | inspect `.devforgeai/sessions/raw-events.jsonl` and the hook dispatcher, then `/<skill> <arg>` |
| any other problem row beginning `COULD_NOT_RUN` | install the missing runner, then `/<skill> <arg>` |
...
| `REQUIRE_HUMAN`, story run, the story itself needs changing | `/clarify <arg>` (the blocked run stays open; `/<skill> <arg>` afterwards re-gates and resumes it, section 3) |
| `REQUIRE_HUMAN`, document run, abandoning | `devforgeai phase fail --reason <text>`, then `/status` |
| `BLOCK`, `WARN` or `OFF`, and the skill's `handoff` set declares no row for this problem kind | `/<skill> <arg> --fix` |
...
No handoff row is a call. `02-skill-roster.md` used to give `plan` a call to `/analyze`, `skill-generator` a call to `/skill-validate`, `retro` a call to `/amend`, `architect` a loop back to `/brainstorm` and `dev` a call to `/clarify`. None of those can nest: `phase start` refuses while a run is active, and there is no operation that suspends a run. Each is a handoff whose first `next` step is that command, run after the run was promoted or abandoned — except `/clarify <story>`, which runs beside a blocked run rather than after it, because a blocked run holds no lease and `clarify` writes only the story document (section 12.2). Even there nothing nests: the primary window prints the block and stops, and the user runs the command.

Research is the exception. Its typed handoff contract is `framework/skills/research/contracts/handoff.md`; on the successful path Research Core writes it and the framework does not restate it. A Research failure returns a typed error, seals nothing, and the framework handoff is rendered from that error, taking `next` from the error's repair route. The rule that a user is never left asking "what's next?" therefore holds on every path.
...
| `junit_dialect` | `generic`, `pytest` or `node` | no | defaults to `generic`. Which runner writes `junit_path`, so the oracle can read that runner's JUnit in the oracle's own vocabulary. `generic` reads the file literally: `<failure>` is a test that ran and failed an assertion, `<error>` is a test that never ran. `pytest` and `node` name runners that report both outcomes through the same element, and the oracle normalises them before it classifies (section 3). A dialect only ever moves an outcome towards `error` or drops a case that never ran; naming one can refuse a transition the literal reading would have admitted, never the reverse |
...
- `junit_dialect: pytest`, because pytest writes an exception raised inside a test body as a `<failure>` whose `message` names the exception. Without the dialect a `NameError` in a `test_plan` test reads as a failing assertion and satisfies the red gate; with it the transition is `COLLECTION_ERROR`. A `node` section names `junit_dialect: node` for the same reason and two more of its own (section 3).
...
| `fix_report` | path | optional; the qa or review report a `--fix` run reads, resolved by `phase start` as the newer of `docs/reports/qa-<arg>.md` and `docs/reports/review-<arg>.md` (section 2). Absent on every run opened without `--fix` |
| `story_sha256` | digest | optional; the digest of the story file as it stood when the gate passed, by the section 3.4 hash rule over the whole file. Present on a story run and on a story-anchored document run, absent otherwise; it is what a resume compares against to decide whether to re-gate (section 3) |
...
| `.devforgeai/work/<run>/evidence/<agent>/findings.md` | Markdown | the sequencer at `devforgeai ingest-result` step 8b, verbatim from the judge receipt's `findings`. No worker writes here and none can: a judge holds no write tool | the next phase's worker by path, the report renderer, the human; named in `<phase>-result.json#findings_path` and the handoff's `findings_paths[]` |
...
`docs/reports/*` is a rendered view, not a second evidence home. A producer writes only inside its candidate root; a judge writes nowhere at all and returns its evidence in the receipt for the sequencer to persist; both return a receipt, and everything under `.devforgeai/` is the sequencer's own.
...
- **evidence file** — the exact path, with `<run>` and `<phase>` substituted. On a judge phase it also names `.devforgeai/work/<run>/evidence/<agent>/findings.md`, which the sequencer writes from the receipt's `findings`.
...
| `smoke` | `smoke_qa` | the phase is a judge: it holds no lease and no write tool, `changed[]` is empty, and the receipt carries a non-empty `findings` | `test_runner_missing: REQUIRE_HUMAN` | `.devforgeai/work/<run>/smoke-result.json`, `smoke-report.md`, `.devforgeai/work/<run>/evidence/smoke_qa/findings.md` written by the sequencer | `report_only`: no file outside the fence changed and the stack policy holds |
| `review` | `dev_critic` | the phase is a judge: it holds no write tool, `changed[]` is empty, the receipt carries a non-empty `findings`, and it is granted no command key | `criterion_without_test: BLOCK` | `.devforgeai/work/<run>/review-result.json`, `review-report.md`, `.devforgeai/work/<run>/evidence/dev_critic/findings.md` written by the sequencer, then `handoff.json` | `report_only`: as `smoke`. On pass this is the last phase: the run becomes `ready_to_promote` and the handoff names `devforgeai promote <run>`; `/review <story>` is the `next` the promotion handoff carries |
...
The primary session stays in the canonical checkout. It never cds into a root, and it never needs to: `devforgeai status` prints the run, the root, the phase, the fence, the granted keys and, when `run.yaml#fix_report` is set, that path as `fix_report`, and the primary pastes that block into the dispatch prompt.
...
Parallel work is parallel runs, not parallel workers: separate stories, separate roots, separate sessions, because Codex's one-open-worker cap is per session. One exemption is deliberate and narrow. A blocked run — `run.yaml#blocked_at` set and no lease held — has no worker in flight and no write in progress, and `clarify`'s fence is the story document under `docs/plan/`, which is disjoint from the code fence the blocked run holds. `/clarify <story>` is therefore admitted against that story while the run stays blocked: it passes `FENCE_OVERLAP` on the fences and is exempted from `STORY_IN_FLIGHT` on the story id (section 2). `review` and `qa` are not exempt — they judge the code the blocked run has not promoted — and `clarify` itself is refused against a story whose run is running, holds a lease, or is `ready_to_promote`. When the blocked run is resumed afterwards, it re-gates against the story `clarify` rewrote (section 3). `phase start` refuses a story whose `write_fence` overlaps the fence of any run that is `active` or `ready_to_promote`, with reason `FENCE_OVERLAP`. The producer-exception paths count as fence members for that test, so two `architect` runs cannot both be open even though their document fences differ elsewhere.
...
3. **The primary window never sees the bytes a run wrote.** Bodies live in the root; the receipt carries paths; the reports carry prose. The one worker output that does enter the primary context is a judge's bounded `findings`, which is prose about the work and not a copy of it, and which the sequencer persists so later readers open a path instead. What stays isolated in every case is the worker's transcript, its reads, its tool traffic and its reasoning. That is what keeps a long run inside a terminal session's context.
