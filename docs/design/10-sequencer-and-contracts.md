...
`<phase>-report.md` is rendered from the same record: skill, phase, status, run, agent and agent id, session, timestamp, reason code, requested rewind, resolved digests, note, issues, the `findings_path` when the phase was a judge's, the changed paths and their kinds, the checkpoint, and the oracle record: each key the sequencer brokered at ingest with its classification, exit and per-criterion outcomes, the verdict and the problem rows, written to `<phase>-result.json#oracle` before the report is rendered and rendered as the report's `## Oracle` section from that record alone, never from the worker's note. It is a rendering, not a second source of truth. At a passing transition the sequencer copies it to `docs/reports/<skill>-<run>-<phase>.md`, which is a canonical path outside the candidate root: the rendered view is the one thing a run writes canonically before promotion, and it is written by the sequencer, so it is never in a candidate diff and never reported as worker-caused change.
...
| `BLOCK`, `WARN` or `OFF`, and the skill's `handoff` set declares no row for this problem kind | `/<skill> <arg> --fix` when the skill declares a fix-report source, else `/<skill> <arg>` |
...
| `fix_report` | path | optional; the report a `--fix` run reads, resolved by `phase start` from the skill's registry fix-report sources: for `dev` the newer of `docs/reports/qa-<arg>.md` and `docs/reports/review-<arg>.md`, for `skill-generator` `docs/reports/validate-<arg>.md` (section 2). Absent on every run opened without `--fix` |
