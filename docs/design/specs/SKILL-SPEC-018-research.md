...
date: 2026-09-02
depends_on:
  - source: framework/skills/research/capability.md#deterministic-research-core-interface
  ...
  - source: framework/skills/research/workflow.md#invocation-and-persistence-authority
  ...
  - source: framework/skills/research/workflow.md#completion-and-non-success-boundary
    ...
    excerpt: |
      ...
      `COMPLETE`.
  - source: framework/skills/research/contracts/delegation.md#worker-roles
  ...
  - source: framework/skills/research/contracts/handoff.md#required-handoff-fields
...
| R9 | discovered | Do not manufacture a failure handoff. `framework/skills/research/workflow.md` records that the current Core exposes no operation that appends a non-success terminal event and no path that seals a partial or failed run. |
...
- **Result:** `RUN-000001` exists at P0 with its request bound and nothing invented; the user has the exact error and the repair route from `framework/skills/research/workflow.md`'s repair-routing table.
...
seal receipt: none
repair route: THE ROW FROM framework/skills/research/workflow.md REPAIR ROUTING
```

### Return envelope

Not applicable, and deliberately so. Research specifications do not use `devforgeai.worker-result/v1`; they reference the typed statuses and records under `framework/skills/research/`. The relevant closed vocabularies are:
...
1. Refuse anything that is not the exact invocation with a slug, a `--request` path and a `--confirm-request` digest — why: no deterministic request builder exists, so a short-form question cannot be turned into a request without inventing fields, and `framework/skills/research/workflow.md` makes missing fields non-inferrable. `references/invocation.md` holds the argument contract.
...
`01-skill-anatomy.md`'s seven sub-phases do not apply. The anatomy document states the exemption in its own first paragraph: Research is governed instead by `framework/skills/research/`, its P0-P9 state machine and typed records are normative, it defines contracts for four worker roles that only read, and deterministic Research Core is its sole canonical writer. The framework mapping below is recorded only so a generator does not try to impose the seven sub-phases on it.
...
Four roles that only read are defined by contract. **None of them is a dispatch target in this build.** `framework/skills/research/contracts/delegation.md` records that the three provider worker-result schemas, the trusted worker broker, and the worker-status-to-reconciliation mapping do not exist, that Core does not launch provider workers or import their results, and that a current provider adapter must stop before its first worker call. These contracts ship as `references/workers.md`, not as `agents/*.md`; section 9 records that decision and its reason. These contracts predate the two-role tool vocabulary and keep the `[read]` shorthand, which is `Read`, `Grep`, `Glob` and `Bash`: `tools` names tools only — a Claude Code subagent's `tools:` frontmatter accepts tool names and MCP server patterns, never a command pattern — so were these roles ever dispatched, the hook dispatcher would be the only command-level bound, admitting `devforgeai status` and its read-only command set (`cat cmp cut diff echo grep head jq ls pwd rg sha256sum tail test tr wc`, plus read-only git subcommands inside the root) and nothing else, because all four write nothing.
...
inputs: [the worker envelope required by framework/skills/research/contracts/delegation.md, the planned lane id, the question id]
...
| OI-8 | The registry's canonical worker names do not apply: Research has no registry phase and no `agent_type` comparison. The four names in section 7 are the contract names from `framework/skills/research/contracts/delegation.md`. |
...
|--------|--------------|------------|-----------|-------|
| claude | `.claude/skills/research/` | exactly `/research` with a slug, `--request` and `--confirm-request`, with no implicit persistence | none installed; the four roles in `references/workers.md` are contracts, not dispatch targets | `argument-hint` and `disable-model-invocation` are compiled into this target's SKILL.md only; `disable-model-invocation` is what stops an implicit match from opening a run. Reconcile against `providers/claude/skills/research/SKILL.md`. |
| codex | `.agents/skills/research/` | exactly `$research` with the same three arguments, with no implicit persistence | none installed; same as above | Portable frontmatter only; invocation policy goes in target-side configuration. Reconcile against `providers/codex/skills/research/SKILL.md`. |
...
Research is validated against `framework/skills/research/` and its typed schemas rather than against the anatomy checks: the sub-phase, persona-and-critic, and `must_not`-per-agent checks that `skill-validator` applies to an anatomy skill do not apply here. What applies is that the ten operation names are used and no others, that the framework envelope appears nowhere, that no step writes a canonical record through a file tool, that the outcome table covers every reserved handoff result as unemitted, and that the description authorises persistence only from the exact explicit invocation.

## 15. Provenance

| Source | Hash | Used for |
|--------|------|----------|
| `framework/skills/research/capability.md#deterministic-research-core-interface` | sha256:fa39d183989c3077bba6c88e5e910464959bac17b51207c6f737ffea2cfdaec9 | sections 1, 6, 7 (the ten operations), 8, 11 |
| `framework/skills/research/workflow.md#invocation-and-persistence-authority` | sha256:4f67a0794bc12c077ea12fcd42b5fe46cc6b00d4f90a4ea5426865e597f6c4ec | sections 2, 3, 4, 7 (steps 1-6), 9 |
| `framework/skills/research/workflow.md#completion-and-non-success-boundary` | sha256:0def4c98f48416d6999a9705becc53bf1b1dc49982094a0f32afe6c1f4ec6802 | sections 6 (failure shape), 7 (handoff outcomes), 9 |
| `framework/skills/research/contracts/delegation.md#worker-roles` | sha256:58d35de2626bac90b4a907778e28331b34851083914d377d2240789c49b26ac8 | sections 7 (worker contracts), 8, 9 (the agents decision) |
| `framework/skills/research/contracts/handoff.md#required-handoff-fields` | sha256:ce7cbf65da6fc3be59ca0e032607ea16df3f7b2f35348bffb6ae7debc3ac40ac | sections 6 (receipt), 7 (close order and outcomes) |
