...
|------|-------------------------|--------|-------|-------------|
| `cross_referencer.md` | `cross_referencer` | none | Read, Grep, Glob, Bash(devforgeai status) | `.claude/agents/analyze-cross_referencer.md`, `.codex/agents/analyze-cross_referencer.toml` |
| `orphan_finder.md` | `orphan_finder` | none | Read, Grep, Glob, Bash(devforgeai status) | `.claude/agents/analyze-orphan_finder.md`, `.codex/agents/analyze-orphan_finder.toml` |
| `stale_hash_finder.md` | `stale_hash_finder` | none | Read, Grep, Glob, Bash(devforgeai status) | `.claude/agents/analyze-stale_hash_finder.md`, `.codex/agents/analyze-stale_hash_finder.toml` |
...
| Kind | Value |
|------|-------|
| Tools | SKILL.md: `Read`, `Agent`, and a Bash grammar no wider than the five model-callable operations `devforgeai status \| phase start <skill> <arg> \| phase fail --reason \| validate \| promote <run>`, the last of which `SKILL.md` calls only after the user asks for the promotion the run's `REQUIRE_HUMAN` block names. Judges: `Read`, `Grep`, `Glob` and `Bash(devforgeai status)`, with no `Write`, no `Edit` and no `apply_patch`; their findings are returned in the receipt and persisted by the sequencer. Producers: `Read`, `Grep`, `Glob`, `Bash(devforgeai status)`, `Edit` and `Write` (Codex `apply_patch`) inside the candidate root, plus `Bash(devforgeai run *)` for the stack keys the phase grants, of which `analyze` grants none. |
| MCP servers | none |
