...
depends_on:
  ...
  - source: docs/design/05-subagent-sets.md#worked-example-dev-tdd
    hash: sha256:37a728c468670a915baa80b635a43e8f44855b555948dcb6ab32a6851361abc7
...
Every `dev` run works inside one candidate root. The sequencer creates it at `devforgeai phase start`, records its path and mode in the run file, and owns it until promotion or abandonment. The three producing workers — `red_dev`, `green_dev`, `refactor_dev` — write their files there with `Edit` and `Write` and run the phase's granted stack keys through `devforgeai run <key>`, which executes with the candidate root as its working directory. The two judging workers — `smoke_qa` and `dev_critic` — declare `writes: none`, carry no write tool at all, and read the checkpoints and oracle output the sequencer recorded. Each returns its detailed evidence in the receipt's `findings` string, and the sequencer persists that string verbatim to `.devforgeai/work/<run>/evidence/<agent>/findings.md`, a run-scoped path the worker cannot choose, gitignored and never promoted. Code reaches the canonical checkout only when the user runs `devforgeai promote <run>` on a run the last passing transition marked `ready_to_promote`; promotion is never automatic and is never part of Handoff.

Two `writes` vocabularies meet here and mean different things. The registry phase's mode — `tests`, `code`, `none`, `docs` — says what a phase may change inside the candidate root. The worker header's `writes` — `candidate` or `none` — says whether that agent is given a write tool at all. A judge is `writes: none` in both vocabularies: it changes nothing inside the root, and it carries no `Write`, `Edit` or `apply_patch` anywhere. Its evidence reaches disk only through the `findings` string the sequencer persists on its behalf.
...
5. **Eval mode** is given in the prompt and is one of two values:
   ...
   - `quick`: write the skill and `<output-dir>/dev/evals/evals.json`, which is a build-time artefact and is excluded from the installed skill (section 8). Build each eval's workspace as section 10 specifies, run each test prompt once with the skill (no baseline run), grade with the grader agent, write `grading.json`, and report pass/fail per expectation in your final message. Do not run the description-optimization loop. Run the eval executions and the grader as foreground Agent-tool subagents, never as background shell processes, and do not end your turn until every `grading.json` exists: a headless session terminates when the top-level turn ends and orphans anything still running.
...
| purpose | Turn one gated story into applied code and tests through five sequencer-checked phases, so the primary context window opens no artifact file — the only worker output that reaches it is the receipt, including a judge's bounded `findings` string — and no phase advances on a worker's own say-so. |
...
| R12 | discovered | A judging worker writes nothing. It returns its detailed evidence in the receipt's `findings` string, at most 16,384 UTF-8 bytes, and the sequencer persists that string verbatim to `.devforgeai/work/<run>/evidence/<agent>/findings.md` at the identity-bound `SubagentStop`; `issues[]` stays the bounded routing summary the handoff and the reports quote. The worker chooses neither the path nor the name, and the persisted file is gitignored, outside the candidate root and the fence, and never promoted. Source: write-model decision D13. |
...
- **Steps:** 1. `--fix` opens a fresh run from phase 1; it does not resume the closed one. 2. The adapter forwards `--fix` to `devforgeai phase start dev STORY-007 --fix`; the sequencer resolves `docs/reports/qa-STORY-007.md` and `docs/reports/review-STORY-007.md`, records the ones that exist in `run.yaml#fix_report`, and prints each as a `fix_report` line in the `devforgeai status` block. The adapter adds nothing to the dispatch prompt, which stays the story id, that block and the phase line. 3. Reading the block, `red_dev` tightens only the tests for the criteria the report marks failed and `green_dev` changes only the code those tests reach. 4. The remaining phases run unchanged.
...
| phase result | JSON, the validated envelope plus the sequencer's added fields, among them `oracle`: the sequencer's own record of the transition check it ran at ingest | `.devforgeai/work/<story>/<phase>-result.json` | none; written by the sequencer |
...
`assets/dev-notes.md` ships as a byte copy of the registry template `docs/design/templates/dev-notes.md`, its template header and its placeholder tokens included. A template is not an instance: the double-brace tokens and the sample `STORY-000` ids are its content, not leftovers waiting to be filled, so the forbidden-text scan in section 14 excludes `assets/` and scans every other directory of the package.

Two `status` vocabularies meet in this file and are different fields. The `status` in `dev-notes` frontmatter is the document lifecycle the registry template declares — `draft | ready | superseded` — and the registry template is authoritative for it. A phase's status is the worker-result closed set — `pass | fail | needs_user | could_not_run` — and it lives in `<phase>-result.json` and in the receipt, never in the notes frontmatter.

The `dev-notes` shape the sequencer's phase report fills:
...
- the sequencer's own record of the transition check: each key it brokered at ingest with its classification and exit, the per-criterion test outcomes, the verdict and the problem rows, rendered from `<phase>-result.json#oracle` and never from the worker's note
```
...
findings: "the judge's detailed evidence"  # judges only, at most 16384 UTF-8 bytes: required on pass or fail, optional on needs_user or could_not_run
next: "red"                             # optional; only with status fail, only from green or refactor
```

The `reason_code` set separates infrastructure failures that look alike from the primary window. `provider_tool_refused` is the provider declining a tool call before any DevForgeAI hook ran — the case that made this specification's judges `writes: none`. `prerequisite_missing` is a worktree-mode prerequisite the `SessionStart` self-test found absent. `checkpoint_fault` is a checkpoint the sequencer could not create, read or reset. `hook_fault` stays reserved for a missing worker identity on the stop event or a malformed receipt. All of them roll up to `INFRA_FAILURE`.

`gate_policy` (`BLOCK | REQUIRE_HUMAN | WARN | OFF`) is a defect-to-action map declared in the consumed story, never a status returned here. An unknown key is refused, as is a `next` value from a phase that declares no `rewind_to`. A judging phase returns the same object with an empty `claimed_paths` and a `findings` string carrying its working; it writes no file, so nothing it does is ever part of `changed`. `findings` is required when a judge returns `pass` or `fail`, because those are the statuses whose working the next phase reads, and optional when it returns `needs_user` or `could_not_run`, where the judge may have nothing to report; the same 16,384-byte bound applies either way. A producer never carries it on any status. At the identity-bound `SubagentStop`, after the receipt validates, the sequencer writes the decoded `findings` verbatim to `.devforgeai/work/<run>/evidence/<agent>/findings.md` — a fixed path the worker cannot choose or name — and the next phase's worker, the handoff and `<phase>-result.json` reach it by that path. That is persistence of a returned result, not a merge into the tree. A `findings` string over 16,384 UTF-8 bytes is refused like any other receipt defect and is never truncated; a `findings` key on a producer receipt is refused as an unknown key would be.

The `findings` body reaches the primary window as part of the subagent's result, exactly as any subagent result does: a hook can validate a final message but cannot suppress it. What stays isolated is the worker's transcript, its file reads, its tool traffic and its intermediate reasoning. The receipt still carries no code and no file body, and a producer returns no `findings` at all.
...
2. Run `devforgeai phase start dev <story-id>`, appending `--fix` and `--lenient` only when the user supplied them. Both flags are forwarded to the sequencer rather than kept in the adapter: `--fix` is what makes the sequencer resolve `docs/reports/qa-<story>.md` and `docs/reports/review-<story>.md`, record the ones that exist in `run.yaml#fix_report`, and narrow the `red` oracle to the criteria they mark failed. Exit 0 opens the run, creates the candidate root and prints the run id, the candidate root, the active phase, the fence, the granted keys and, on a `--fix` run, each recorded fix report path; exit 1 is a gate refusal with the defect list on stderr; exit 2 is a usage error; exit 3 is a missing runner — why: the gate is inlined in this one operation, so there is no way to open a phase without it, and the root exists before any worker can be dispatched into it.
...
4. Dispatch the worker the sequencer named, in its own context window. The prompt has exactly three parts and nothing else: the story id, the `devforgeai status` block pasted verbatim, and one line naming the phase. The status block names `run`, `candidate.root`, `phase`, `fence` and `granted_keys`, and on a `--fix` run it additionally prints one `fix_report` line per path `run.yaml#fix_report` records; that line is the only route by which the qa or review report path reaches a worker. Add no previous phase's report path — a worker derives `.devforgeai/work/<run>/<phase>-report.md` from the run id the block already names — why: restating a goal or a criterion into a worker prompt replaces the gated artifact with a paraphrase, and the status block is the one place the root, the granted keys and the fix report are stated, so a worker never guesses where it may write or which report bounds its work.
...
Bash grammar for this skill is exactly `devforgeai status`, `devforgeai phase start dev <story> [--fix] [--lenient]`, `devforgeai phase fail --reason <text>`, `devforgeai validate` and `devforgeai promote <run>`. `devforgeai session-start`, `devforgeai ingest-result`, `devforgeai phase next`, `devforgeai run <key>` and the `devforgeai candidate` operations are hook-only and refuse a call that carries no `DEVFORGEAI_HOOK_EVENT` marker. A worker's `devforgeai run <key>` call is the one exception the hook layer admits: it is allowed to the lease holder, for a key the phase granted, with the candidate root as its working directory.

Two rules bind every one of those calls and every dispatch, and both are compiled into `SKILL.md`:

- Each call is the bare command and nothing else. No `; echo`, no `&&`, no `||`, no pipe, no `>` or `2>` redirect, no subshell, no wrapping in a here-document. The dispatcher's single-argv rule denies a compound before it runs, so a command that would otherwise be legal is refused for its punctuation; read the sequencer's own output instead of arranging for a shell to echo something after it.
- On any block, refusal or non-zero exit, print the handoff block the sequencer rendered — or, when the gate refused before a run opened, the sequencer's stderr plus the section 7f row — and stop there. Do not open a hook file, the dispatcher, `devforgeai.py`, `policy.py`, a log under `.devforgeai/`, or any other session's state to work out why; do not send a message to another session; do not write to memory. The refusal is the finished result of the run, and the next command belongs to a human.
...
| 5 | Work: `smoke` | worker: `smoke_qa` | none |
| 6 | Review | worker: `dev_critic` | none |
...
| `smoke` | `smoke_qa` | `writes: none`: the worker carries no write tool at all, `claimed_paths` is empty and the `refactor` checkpoint diff shows nothing changed since it; the receipt carries a `findings` string — required on `pass` or `fail`, optional on `needs_user` or `could_not_run` — which the sequencer persists to `.devforgeai/work/<story>/evidence/smoke_qa/findings.md` after the receipt validates | `test_runner_missing: REQUIRE_HUMAN`, which cannot fire here because the phase's `report_only` oracle spends no key | `.devforgeai/work/<story>/smoke-result.json`, `.devforgeai/work/<story>/smoke-report.md` | `report_only`: nothing changed in the root since the `refactor` checkpoint and the whole-tree package and import policy holds |
| `review` | `dev_critic` | as `smoke`, with its `findings` persisted to `.devforgeai/work/<story>/evidence/dev_critic/findings.md`; the phase grants no command key, so `devforgeai run` is refused for this worker on the key as well as on the lease | `criterion_without_test: BLOCK` | `.devforgeai/work/<story>/review-result.json`, `.devforgeai/work/<story>/review-report.md`, then `.devforgeai/work/<story>/handoff.json` | `report_only`: as `smoke`. On pass this is the last phase: the run is marked `ready_to_promote`, enforcement is cleared, and the first handoff block names `devforgeai promote <run>`; the second block, written by that command after the user asks for it, names `/review <story>` |
...
- Under `--fix`, the qa report and review report paths are inputs, and they reach you one way only: as the `fix_report` lines of the `devforgeai status` block in your dispatch brief, printed from `run.yaml#fix_report`, which the sequencer filled at `phase start`. Whichever of the two exists is listed there — `qa` routes here on `verdict: fail` and `review` routes here on `verdict: findings`, so a `--fix` run after a review with no qa run yet has only the review report. Do not go looking for a report the block does not name. Tighten or add tests only for the criteria the qa report marks failed and the findings the review report raises, and leave every other planned test byte-identical, because the oracle compares digests, not intentions.
...
The `smoke` phase answers one question: does this story work, criterion by criterion. You judge and you write nothing: you have no `Write`, no `Edit` and no `apply_patch`, and the candidate root is byte-identical when you finish. Your detailed evidence goes in the receipt's `findings` string, and the sequencer persists it for you at `.devforgeai/work/<story>/evidence/smoke_qa/findings.md` once the receipt validates.

- The evidence already exists. Read `.devforgeai/work/<story>/green-report.md` and `.devforgeai/work/<story>/refactor-report.md`: the sequencer renders each from the accepted result and includes the oracle's classification and problem rows. The sequencer runs every oracle at ingest, so a second run from this worker would add nothing; the phase grants no key, this worker holds no lease, and it carries no write tool at all, so neither a stack command nor a write reaches disk.
- The subject is the `refactor` checkpoint of the candidate root. Read the files there; they are the story as `refactor` left it. You put nothing there and nothing anywhere else: the path the sequencer persists your `findings` to is outside that root, so nothing you report can reach the fence or the promotion.
- Put the criterion-by-criterion table in `findings` and keep `issues` to the bounded summary: one row per failing or uncovered criterion, at most ten. `findings` is where a reader goes for the working; it is capped at 16,384 UTF-8 bytes and is refused rather than truncated if it exceeds that. `issues` is what the handoff and the phase report quote. Do not try to record the table under another filename, as JSON, or through a shell redirect: a judge that is refused a write has no write to make.
...
The `review` phase is the independent critic. You judge, you repair nothing, and you write nothing: you have no `Write`, no `Edit` and no `apply_patch`, and the candidate root is byte-identical when you finish. Your detailed evidence goes in the receipt's `findings` string, and the sequencer persists it for you under this run's evidence directory, at `.devforgeai/work/<story>/evidence/dev_critic/findings.md`, once the receipt validates.
...
- Report defects; do not fix them. This phase changes nothing anywhere, grants no command key and runs nothing. The criterion-to-test-to-code map you walked belongs in `findings`, capped at 16,384 UTF-8 bytes and refused rather than truncated if it exceeds that; `issues` carries one bounded row per defect, at most ten. Do not try to record the map under another filename, as JSON, or through a shell redirect: a judge that is refused a write has no write to make.
...
inputs:
  ...
  - the qa or review report the status block names on its fix_report lines, only when the run was opened with --fix
outputs:
...
outputs:
  - a receipt whose findings string carries one row per criterion with its test, verdict and the report line that proves it, which the sequencer persists under this run's evidence directory
  - a receipt whose issues carry one bounded row per failing or uncovered criterion and whose evidence_refs name the reports the verdicts were read from
must_not:
...
outputs:
  - a receipt whose findings string carries the criterion-to-test-to-code map it walked, which the sequencer persists under this run's evidence directory
  - a receipt whose issues carry one bounded row per defect, each quoting the test name, changed path or report line that evidences it, and whose evidence_refs name the reports it walked
must_not:
...
Row selection is implemented, not defaulted. These rows are the skill's `handoff` set in the sequencer's registry (`examples/hooks/policy.py`), and a phase's refusal names its row from that set rather than falling through to a generic default. That is why a `red` failure whose `issues` carry kind `already-green` routes to `/clarify {story}` and not to the exhausted-budget row below it (D13 item 7).
...
| `fail` at `red` on a plain run because a planned test already passes (`issues` kind `already-green`), attempts exhausted (`REQUIRE_HUMAN`) | `/clarify {story}` — a human decides whether the criterion is redundant or the story needs re-slicing | sequencer, selecting this row from the registry `handoff` rows |
...
| `could_not_run`, `reason_code: network` (a brokered command needed the network and could not reach it) | restore connectivity or make the command offline-capable, then `/dev {story}` | sequencer, through the same missing-runner route |
| `could_not_run`, `reason_code: provider_tool_refused` (the provider declined a worker's tool call before any DevForgeAI hook ran) | report the refused call to the spec author, then `/dev {story}` — a worker asking for a tool its role does not carry is a specification defect, not a project fault | sequencer, through the same missing-runner route |
| `could_not_run`, `reason_code: prerequisite_missing` (a worktree-mode prerequisite the `SessionStart` self-test names) | install or repair the named prerequisite — git, a first commit, the ignore rule, the tracked settings or `stack.yaml` — then `/dev {story}` | sequencer, through the same missing-runner route |
| `could_not_run`, `reason_code: checkpoint_fault` (the sequencer could not create, read or reset a checkpoint) | inspect the candidate root under `.devforgeai/work/{run}/wt`, then `devforgeai phase fail --reason <text>` to abandon it and `/dev {story}` to start again | sequencer, through the same missing-runner route |
| `could_not_run`, `reason_code: hook_fault` (no worker identity on the stop event, or a malformed receipt) | install or repair the hook dispatcher, then `/dev {story}` | sequencer, through the same missing-runner route |
...
| `smoke_qa` | Dispatch when `devforgeai status` names phase `smoke` of a `dev` run; it decides each acceptance criterion from the recorded oracle output and writes nothing, returning its working in the receipt's findings string. |
| `dev_critic` | Dispatch when `devforgeai status` names phase `review` of a `dev` run; it reports coverage and traceability defects with quoted evidence and writes nothing, returning its working in the receipt's findings string. |

The frontmatter of each compiled file is the full key set `templates/agent-md.md` fixes, not a four-key subset: `name`, `description`, `tools` and `writes` (the template's `required_frontmatter`), then `skill`, `responsibility`, `inputs`, `outputs`, `must_not`, `isolation`, `returns` and `provenance`. Every one of those but `description`, `isolation` and `provenance` is copied from the worker's section 7e contract, which additionally carries `granted_keys`; `description` comes from the table above, `isolation` is `required` for all five, and `provenance` is one entry naming this specification's section 7 with its digest. skill-validator refuses an agent file with no `responsibility`, no `must_not` or no `writes`, and refuses `tools` that do not match the role `writes` declares, which is why the short set is not enough.

Two keys named `isolation` must not be confused. The template's `isolation: required` is the worker-contract statement that this phase runs in its own context window. Claude's git-worktree isolation key is a different key with a different meaning and is omitted from every compiled file, as the provider-difference list below states. `model` is a Claude-target key the template does not require; it is `inherit` for all five and is written only into the Claude file.
...
- The Codex file carries `name`, `description`, `sandbox_mode`, `approval_policy` and `developer_instructions`. `sandbox_mode` is the writable-workspace mode for the three producers and the read-only mode for the two judges, which is Codex's equivalent of the `tools` split; `apply_patch` is the write tool the producers use in place of `Edit` and `Write`, and the judges carry no write tool on either target.
- Neither provider carries the lease, the fence or the granted keys in the agent file. They live in `.devforgeai/work/<run>/run.yaml` and are enforced by the hook dispatcher, so a stale agent file cannot widen what a worker may write. A producer's write is admitted by path — under the candidate root and inside the fence — not by the tool list alone.
- The two judges run read-only on both targets: no `Write`, no `Edit` and no `apply_patch` in the tool list, and Codex's read-only sandbox mode. The provider itself refuses a report-shaped file write from a subagent on Claude, undocumented and relied on in neither direction, so the design does not depend on a hook catching it: a judge has no write tool to be caught with, and its evidence reaches disk only as the `findings` string the sequencer persists.
...
dev/SKILL.md               # at most 500 lines: identity, phase list, dispatch loop, handoff table
  ...
  assets/dev-notes.md      # the dev-notes template, placeholders and all
```

One directory the generator creates is not part of that layout. `evals/evals.json` is written to `<output-dir>/dev/evals/` in `quick` mode and is build-time only: it is generation feedback, it is excluded from the installed skill, and an installer that copies the package leaves it behind. The layout above is what installation carries; `evals/` is what generation leaves beside it.
...
| File | Used for |
|------|----------|
| `dev-notes.md` | the `dev-notes` template `dev` owns, shipped as a byte copy of `docs/design/templates/dev-notes.md`: its template header, its double-brace placeholder tokens, its frontmatter keys and the Note, Issues, Files and Oracle sections the phase report is rendered into. A template carries placeholders by design, so section 14's forbidden-text scan excludes this directory |
...
| `smoke_qa.md` | `smoke_qa` | none | `.claude/agents/dev-smoke_qa.md`, `.codex/agents/dev-smoke_qa.toml` |
| `dev_critic.md` | `dev_critic` | none | `.claude/agents/dev-dev_critic.md`, `.codex/agents/dev-dev_critic.toml` |
...
| The skill declares `handoff.outcomes` and an author expects the sequencer to select a row from it | `01-skill-anatomy.md` says the sequencer selects the row by envelope status and fills the placeholders from state, but `examples/hooks/devforgeai.py` selects from its own default table in `10-sequencer-and-contracts.md` section 6 and never reads the skill's block | Section 7f marks which rows the sequencer renders and which the adapter prints from a gate refusal. Selection is implemented, not defaulted: the section 7f rows are this skill's `handoff` set in the sequencer's registry (`examples/hooks/policy.py`), a refusal names its row from that set, and the adapter prints the rendered block verbatim. The `red` failure whose `issues` carry kind `already-green` therefore routes to `/clarify {story}`, which is the row this specification has always stated. **Decision (D13 item 7):** the sequencer was wrong to fall through to a generic row, and it is the sequencer that changed, not this table. What remains unimplemented is reading the rows from the skill's own `skill.yaml` `handoff.outcomes` block rather than from the registry; the two are required to agree, and nothing here gates on the former. |
...
| An author reads "no artifact content enters the primary context window" as covering a judge's findings | The claim is false on both providers — a subagent returns its result to the parent, and a hook can validate a final message but cannot suppress it — so the design rests on something the runtime does not do | The bounded `findings` body does enter the primary window, as part of the subagent's result, exactly as any subagent result does. What stays isolated is the worker's transcript, its file reads, its tool traffic and its intermediate reasoning. **Decision (D13 item 4):** the guarantee this specification makes is that the primary window opens no artifact file and that no receipt carries code or a file body; a producer returns no `findings` at all. Every sentence claiming a body never reaches the primary context is replaced by that one. |
| The compiled agent file is expected to carry the fence, the lease or the granted keys | An installed file drifts from the run file and an author treats it as authority | The agent file carries the `templates/agent-md.md` key set — `name`, `description`, `tools`, `writes`, `skill`, `responsibility`, `inputs`, `outputs`, `must_not`, `isolation`, `returns`, `provenance`, plus `granted_keys` from the section 7e contract and `model` on the Claude target — and the body; the fence, the lease and the granted keys as *enforced values* live in `.devforgeai/work/<run>/run.yaml`, so the `granted_keys` line in an agent file is a statement of the role, never the authority the dispatcher checks. **Decision (section 7g, D14 item 4):** section 7g's earlier four-key list was a defect and is corrected to the template's full set; every Claude-only key — hooks, memory, background, `permissionMode`, `maxTurns`, `effort`, `disallowedTools`, `mcpServers`, colour and the git-worktree isolation key — is omitted, and `skills:` preloads nothing, so the two targets compile from one contract. The template's `isolation: required` is the worker-contract key and is unrelated to Claude's git-worktree isolation key. |
...
| An eval workspace installs the skill but not the compiled agent files | The `Agent` tool has no `red_dev` to dispatch, the first phase never starts, and the eval fails for a reason the skill did not cause | The workspace build compiles the five section 7g Claude agent files into `<copy>/.claude/agents/dev-<role>.md` before the prompt runs. **Decision (D14 item 1):** the compiled agents are part of the workspace, not an installation step deferred to a human; section 10's recipe carries the step. |
| An author has to guess how a `--fix` run's report path reaches a worker | Section 7a step 4 said to add the report path to the prompt, while the three-part dispatch prompt rule says the prompt is the story id, the status block and the phase line, so two rules contradict each other | `--fix` is forwarded to `devforgeai phase start`, which resolves `docs/reports/qa-<story>.md` and `docs/reports/review-<story>.md`, records the ones that exist in `run.yaml#fix_report`, and prints one `fix_report` line per recorded path in the `devforgeai status` block. The prompt stays exactly three parts. **Decision (D14 item 2):** the status block is the single channel; the adapter adds nothing to it, and the sequencer, not the adapter, holds the fix state. |
| `assets/dev-notes.md` is stripped of its placeholders to satisfy a forbidden-text scan | The shipped asset stops being a template: the renderer has no shape to fill and the registry's `dev-notes` contract is not what the package carries | The asset is a byte copy of the registry template, placeholders and template header included, and section 14's scan excludes `assets/` and covers every other directory. The `status` in `dev-notes` frontmatter is the registry template's document lifecycle (`draft | ready | superseded`); a phase's `pass | fail | needs_user | could_not_run` is the worker-result vocabulary and lives in `<phase>-result.json`. **Decision (D14 item 3):** templates carry placeholders by design, and the registry template is authoritative for its own frontmatter vocabulary. |
| `references/<phase>.md` must be section 7d verbatim, and an author cannot tell whether a heading is a violation | Either every reference file is a headless fragment, or the generator adds framing the "verbatim" rule appears to forbid and reports a deviation | One `# <phase>` heading line above the verbatim text is allowed and is the only permitted addition. **Decision (D14 item 5):** stated in section 0 rule 8 and in section 8's `references/` table. |
| `metadata.provenance.hash` is described rather than specified | A generator writes the sentence describing the hash into the frontmatter as if it were the value | The hash is computed with the `docs/design/01-skill-anatomy.md#context-bundle-format` rule over `docs/design/specs/SKILL-SPEC-001-dev.md#3-description`, and the recorded value is `sha256:` followed by 64 hex characters. **Decision (D14 item 6):** section 12's placeholder sentence is replaced by that rule reference. |
| `evals/evals.json` is treated as part of the installed skill | The package ships a directory no provider reads, and the layout in section 8 is not what installation carries | `evals/` is written to `<output-dir>/dev/evals/` in `quick` mode, is generation feedback only, and is excluded from the installed skill. **Decision (D14 item 7):** section 8's layout is the installed package; `evals/` is build-time only. |
| A compiled `SKILL.md` wraps a sequencer call to capture or confirm its output | The dispatcher's single-argv rule denies the compound, so `devforgeai phase start dev STORY-001 --lenient; echo done` is refused for its punctuation and the run never opens | Every dispatch and every sequencer call is the bare command: no `; echo`, no `&&`, no `\|\|`, no pipe, no `>` or `2>` redirect, no subshell, no here-document. **Decision (D14, SKILL.md defect 1):** section 7a states it after the grammar paragraph, and it is compiled into `SKILL.md`. |
| A compiled `SKILL.md` investigates a block instead of printing it | The primary window opens `dispatch.py`, `policy.py` or a run log to diagnose a refusal, fills with content it cannot unload, and sometimes acts on a guess the sequencer already refused | On any block, refusal or non-zero exit the skill prints the rendered handoff — or the sequencer's stderr plus the section 7f row when the gate refused before a run opened — and stops. It never opens a hook file, the sequencer sources or a log, never messages another session, and never writes memory. **Decision (D14, SKILL.md defect 2):** the refusal is the finished result of the run, and the next command belongs to a human. |
...
- Every `devforgeai` call in `SKILL.md` is a bare command, with no `; echo`, no `&&`, no pipe, no redirect and no subshell.
- The `SKILL.md` Bash grammar is no wider than `devforgeai status`, `devforgeai phase start dev <story> [--fix] [--lenient]`, `devforgeai phase fail --reason <text>`, `devforgeai validate` and `devforgeai promote <run>`.
...
6. Compile the five section 7g Claude agent files into `<copy>/.claude/agents/`, named `dev-red_dev.md`, `dev-green_dev.md`, `dev-refactor_dev.md`, `dev-smoke_qa.md` and `dev-dev_critic.md`, each with the section 7g frontmatter key set and the four-part body. Without them the `Agent` tool has no worker to dispatch and phase `red` never starts, so the eval fails for a reason the skill did not cause.
7. Install the generated skill at `<copy>/.claude/skills/dev/` and run the prompt from inside the copy.
...
# every call is the bare command: no compound, no pipe, no redirect, no subshell
```
...
|--------|--------------|------------|-----------|-------|
| claude | `.claude/skills/dev/` plus `.claude/agents/` worker profiles | `/dev STORY-NNN [--fix] [--lenient]` | one provider-native subagent per canonical worker name: three producers that write in the candidate root, two judges that carry no write tool and return their evidence in the receipt's `findings` string | Provider-specific frontmatter keys (`argument-hint`, `disable-model-invocation`) are compiled into this target's `SKILL.md` only. |
...
metadata:
  ...
  provenance:
    - source: "docs/design/specs/SKILL-SPEC-001-dev.md#3-description"
      hash: "sha256:0f2c... the 64 hex characters the rule below produces"
```

The `hash` value is computed, not described. Apply the hash rule of `docs/design/01-skill-anatomy.md#context-bundle-format` to the source above: resolve the heading anchor to its section, normalise CRLF to LF, join the section's lines with LF and append one LF, then take `sha256` over the UTF-8 bytes and record it as `sha256:` followed by the 64 hex digits. That is the same rule `run_conformance.section_digest`, `devforgeai.section_bytes` and `specs/verify.py` implement, so a digest produced any other way fails the next re-resolution. Write the computed value into the frontmatter; the string above shows only its shape.
...
grep -l 'writes: none' <output-dir>/dev/agents/*.md      # smoke_qa.md dev_critic.md
# neither judge carries a write tool
grep -LE 'Write|Edit|apply_patch' <output-dir>/dev/agents/smoke_qa.md <output-dir>/dev/agents/dev_critic.md
