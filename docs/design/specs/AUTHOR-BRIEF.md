...
1. **Write permission is per role.** Producers write; judges do not. Producer roles are
   ...
   and `Bash` for the keys the phase granted; judges hold `Read`, `Grep`,
   `Glob` and `Bash`, and no write tool of any kind. A judge creates no
