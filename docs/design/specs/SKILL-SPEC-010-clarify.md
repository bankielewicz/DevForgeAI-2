...
| Kind | Value |
|------|-------|
| Tools | SKILL.md: `Read`, `Agent`, and a Bash grammar no wider than the five model-callable operations `devforgeai status \| phase start <skill> <arg> \| phase fail --reason \| validate \| promote <run>`. Document writers (`question_writer`, `answer_recorder`): `Read`, `Grep`, `Glob`, `Bash` plus `Edit` and `Write`, which Codex serves as `apply_patch`, denied outside `candidate.root` and outside the run's fence. Judge (`ambiguity_finder`): the same read set and no write tool of any kind, its findings returned in the receipt and persisted by the sequencer. No `clarify` phase grants a stack command key, so no worker is granted a `devforgeai run` key. |
| MCP servers | none |
